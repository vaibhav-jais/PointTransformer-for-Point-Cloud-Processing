...
def parse_args():
    ...
    parser.add_argument('--save_npy_predictions', type=str, default='epoch30_npy_predictions_keyframesOnly', help='folder name to save npy preds [default: None]')
...
def generate_confusion_matrix():
    ...
    if args.model == 'PointTransformerNetwork':
        model = PointTransformerSeg(args.num_points)
    else:
    ...
    for data, _ , frame_name in tqdm(val_loader):       # _ -> labels
        # filtering input data to train on keyframes only and exclude the detections from sweeps
        mask = data[:, :, 4] == 0
        inference_data = torch.masked_select(data, mask.unsqueeze(2)).reshape(1, -1, 7)
        print(data.shape, mask.shape, inference_data.shape)
        data = inference_data.to(device=device)
        #data = data.squeeze()
    ...
    for val_scene in val_h5_scenes:
        ...
        with h5py.File(val_file_path, 'r') as val_file:
            ...
            for frame_idx in range(num_frames):
                ...
                for sensor_name in sensors_group.keys():
                    ...
                    if frame_idx <= (num_frames - 2):
                        ...
                        if slice_end != slice_start:
                            ...
                            data_label_sensorwise = np.array(sensor_group['data'][slice_start:(slice_start+((indices[0][-1])+1)), -1])
                ...
                assert true_label_frame.shape == predicted_label.shape, f"label_array of shape {true_label_frame.shape}, predicted_frame_label of shape {predicted_label.shape}"
...
def plot_conf_heatmap():
    ...
    plt.tight_layout()
    conf_file_name = os.path.join(model_logs_current_iteration, 'confusion_matrix_keyframesOnly.png')
