...
def parse_args():
    ...
    parser.add_argument('--num_points', type=int, default=1024, help='Number of input points to the Model [default: 1024]')
    ...
    #parser.add_argument('--log_dir', type=str, default='AblationStudyXYZOnly_training_with_moving_static_class_separated_ImprovedGTLast_1.4_0.4_1.5_0.5', help='Log path [default: None]')
    parser.add_argument('--log_dir', type=str, default='RoughPart', help='Log path [default: None]')
...
def main(args):
    ...
    experiment_dir = experiment_dir.joinpath('PointTransformerBaseline')
    experiment_dir.mkdir(exist_ok=True)
    ...
    shutil.copy('models/point_transformer_blocks_and_layers.py', str(experiment_dir))
    shutil.copy('models/point_transformer_modules.py', str(experiment_dir))
    model = MODEL.PointTransformerSeg(num_points=args.num_points, nneighbors = args.nneighbors).to(device)
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        log_string('**** Epoch %d(%d/%s) ****' % (start_epoch +1, epoch + 1, NUM_EPOCHS))
        ...
        if epoch % 5 == 0:
            ...
            example_input = torch.tensor(example_input[:, :, [0, 1, 2, 7, 8]])
